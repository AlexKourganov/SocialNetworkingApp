...
class Notifications extends Component {
    ...
    render() {
        ...
        if(notifications && notifications.length >0){
            ...
            ?(
                notificationIcon = (
                    <Badge badgeContent={notifications.filter(not => not.read === false).length}  color='secondary' classes={{ badge: classes.customBadge }}>
            ...
            notifications.map(not=>{
                ...
                return(
                    <MenuItem key={not.createdAt} onClick={this.handleClose}>
                        {icon}
                        <Typography component={Link} color='textPrimary' variant='body1' to={`users/${not.recipient}/scream/${not.screamId}`}>
        ...
        return (
            <Fragment>
                ...
                <Menu id='simple-menu' anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={this.handleClose} onEntered={this.onMenuOpened}>
                    {notificationsMarkUp}
                </Menu>
            </Fragment>
        )
